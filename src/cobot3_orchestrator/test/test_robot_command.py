...
# ── YieldToPeer (순찰 중 상대가 hold · scan · pick 이면 선다) ─────────────

class _YieldNode(FakeNode):
    def __init__(self):
        super().__init__()
        self.should = (False, 'state=patrol')
        self.patrol_node = SimpleNamespace(status=Status.INVALID)
        self.patrol_waypoints = SimpleNamespace(idx=3)
        self.suppressed = 0
        self.published = 0
...
def _yield_leaf(node):
    leaf = tm.YieldToPeer('yield_peer', node)
    leaf.initialise()
    return leaf


def test_yield_idle_when_peer_not_using_sim():
    node = _YieldNode()
    assert _yield_leaf(node).update() == Status.FAILURE
    assert node.patrol_waypoints.idx == 3


def test_yield_rewinds_waypoint_once_when_patrol_was_running():
    node = _YieldNode()
    node.should = (True, 'state=pick')
    node.patrol_node.status = Status.RUNNING
    leaf = _yield_leaf(node)
    assert leaf.update() == Status.RUNNING
    assert node.patrol_waypoints.idx == 2          # 가던 정차점으로 재개
    assert leaf.update() == Status.RUNNING
    assert node.patrol_waypoints.idx == 2          # 한 번만 되감는다
    assert node.published == 1
    node.should = (False, 'state=nav')              # 상대 pick 끝
    assert leaf.update() == Status.FAILURE


def test_yield_does_not_rewind_if_patrol_leaf_not_running():
    node = _YieldNode()
    node.should = (True, 'state=scan')
    assert _yield_leaf(node).update() == Status.RUNNING
    assert node.patrol_waypoints.idx == 3


def test_yield_gives_up_after_max(monkeypatch):
    node = _YieldNode()
    node.should = (True, 'state=pick')
    leaf = _yield_leaf(node)
    t = [1000.0]
    monkeypatch.setattr(tm.time, 'monotonic', lambda: t[0])
    assert leaf.update() == Status.RUNNING
    t[0] += tm.PEER_YIELD_MAX_S + 1
    assert leaf.update() == Status.FAILURE
    assert node.suppressed == 1


def _peer_self(stage, age=0.1, failed=False, suppressed=None):
    return SimpleNamespace(
        _peer_camera_stages=tuple(tm.DEFAULT_PEER_CAMERA_STAGES),
        _peer_last_rx=tm.time.monotonic() - age, _peer_stage=stage,
        _peer_failed=failed, _yield_suppressed_stage=suppressed)


def test_peer_uses_sim_rules():
    f = tm.TaskManager.peer_uses_sim
    for st in ('hold', 'scan', 'pick', 'stack_scan', 'stack_pick'):
        assert f(_peer_self(st))[0] is True
    for st in ('patrol', 'nav', 'place', 'yield_peer', None):
        assert f(_peer_self(st))[0] is False       # yield_peer 가 없으니 교착 없음
    assert f(_peer_self('pick', age=tm.PEER_CAMERA_STALE_S + 1))[0] is False
    assert f(_peer_self('pick', failed=True))[0] is False
    assert f(_peer_self('pick', suppressed='pick'))[0] is False
    none_rx = _peer_self('pick'); none_rx._peer_last_rx = 0.0
    assert f(none_rx)[0] is False
