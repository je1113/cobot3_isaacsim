...
알려진 갭
  - 스택 QR 이 화면에서 71 px 밖에 안 된다. 40 mm 라벨을 0.355 m 에서 보는
    셈인데 디코드 실측 하한이 70 px 이다(frames.yaml qr.thresholds_px 는
    100 을 권한다). 관측 자세는 티칭됐지만(packaging_output_scan) 여유가
    없어서, STACK_SCAN 이 여전히 가장 먼저 멈출 자리다. 더 가까이 세우려면
    선반까지 0.09 m 밖에 안 남은 정차점을 더 밀어야 한다.
...
# y 0.271~1.671, 윗면 z 0.54) 위 (3.83, 1.23) 에 yaw -90 도로 놓여 있고,
# 그 -x 면에 40 mm QR 이 붙어 있다.
#
# ★ 마지막 점은 관측 자세를 티칭한 그 자리다. isaacpjt/tools/out/
#   taught_poses.yaml 의 packaging_output_scan 이 base_link (3.45786,
#   1.70424) · yaw 2.4도 에서 떠졌고, 관측 자세는 티칭한 베이스 자세에서만
#   유효하다. 반올림해서 (3.46, 1.70, 0) 을 쓴다 — yaw 2.4도는 nav_server 의
#   도착 정렬 허용오차(5도) 안이라 구분이 안 된다.
#   그 자세에서 손목캠은 (3.409, 1.323, 0.578) 에 서고 QR 까지 0.355 m 다
#   (화면에서 약 71 px. 디코드 실측 하한이 70 px 이라 여유가 없다).
#
# 첫 점은 로더에서 서쪽으로 빠지는 이탈점이다. 로더에서 스택으로 바로 그으면
# 선반 남서 모서리에 차체가 닿는다. 둘째 점에서 남향으로 내려간 뒤 마지막은
# 서쪽에서 동쪽으로 곧게 들어간다 — 그래야 정차점에서 제자리회전이 없다.
#   정차점 회전 여유가 25도뿐이다(선반까지 0.09 m). 비스듬히 들어오면
#   도착 정렬이 거기서 몸을 돌리다 선반을 친다.
...
# 스택 자리 → 검사 스테이션. 선반에서 서쪽으로 후진해 빠져나온 뒤 남하하고,
