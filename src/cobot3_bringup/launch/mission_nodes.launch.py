...
#  robot2 는 같은 규칙을 Shelf_02 와 자기 도크 줄(y=-7.575)에 적용한 것이다.
#
#  ★ 2026-09-23: 시험 씬에서 magazine_2_blue 가 Shelf_02 의 남쪽 면으로
#    옮겨지면서(ae4f06b) robot2 의 통로가 +y 에서 -y 로 바뀌었다. 이제
#    SHELF-B 는 SHELF-A 를 Δy = -4.918677 만큼 y 로 평행이동한 판이라,
#    아래 robot2 의 선반 줄 좌표는 robot1 값 + Δy 이고 yaw 도 robot1 과
#    같은 0 이다(예전에는 180 이었다). 근거는 shelves.yaml 의 SHELF-B 주석.
#
...
# 올라가고, 마지막 구간은 회전 없이 들어간다 — 두 로봇 다 후진이다
# (robot2 가 전진이던 것은 yaw 180 이던 옛 배치 때 이야기다).
#
