...
| # | 내용 | 그림에서 |
|---|---|---|
| 1 | `carrier_pair` 8쌍이 추정값 | `production_tracking` · 회수 산출물 예측이 빈 결과 |
| 2 | payload 없는 로봇 사건(`SCAN` hard 실패 · 일시정지 · 비상정지) | **`robot_log` 가 생기면 표가 7개**가 된다 |
| 4 | 화면이 yaml을 고쳤을 때 노드가 어떻게 아나 | §2-1의 "yaml로 보낸 것"들이 실제로 동작할지를 가른다 |
